...
func TestProfileExcludesTheProvidersThatCannotRunThisModel(t *testing.T) {
	...
	// CoreML was excluded once on a measurement that turned out to be wrong. It runs this model correctly and at
	// roughly CPU speed, and its one real cost - compiling the 6.8 GB graph - is paid once and then cached.
	if contains(p.ExcludeEPs, types.ExecutionProviderCoreML) {
		t.Error("CoreML must not be excluded: it produces correct output at roughly CPU speed")
	}
}
